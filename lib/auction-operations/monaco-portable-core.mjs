/* ════════════════════════════════════════════════════════════════════════
   ...
   ── APPLY IS DELIBERATELY WITHHELD ─────────────────────────────────────
   This module builds a deterministic plan and nothing else. There is no
   writer for this family yet, and the dispatcher refuses it by name (see
   applyDispatchFor in packetContract.ts). A plan from here is reviewed;
   it is never executed by anything in this flight.
 ...
 */
export async function buildPortablePlan({ manifest, keeper, keeperSha256, keeperByteLength, db, packetId }) {
  ...
  /* ── SOURCE ARTIFACT — hash must describe the right object ──────────────
     ONE spec, for the official sale page, with content_hash NULL: this flight
     did not fetch that page, so it holds no hash of it. The keeper's hash is
     the hash of the KEEPER and is recorded on the keeper block below, never
     on a row whose source_url is an official page. */
  const g = keeper.price_basis_governance;
  const artifactSpecs = [
    {
      key: "sale_page",
      ...
      attribution_note:
        ...
        `Facts reconciled in the accepted private keeper (sha256 ${keeperSha256}); the keeper is not this artifact and its hash is not this page's hash.`,
      ...
      omission_statement:
        "Raw Monaco HTML, images and the private keeper bytes are not retained on this artifact. Estimates, specifications, year, session and per-lot canonical URLs have no Auction Evidence column and are retained in the private keeper and in this plan.",
    },
  ];
  ...
  const plan = {
    ...
    keeper: {
      ...
      retention: "STAGING_ONLY — not durably retained; private retention blocker OPEN",
      source_artifact_representation: "UNRESOLVED — a private keeper has no truthful source_url; see the flight return",
    },
    house,
    sale: view.sale,
    ...
    contradictions,
    apply: { enabled: false, reason: withheldReason },
    summary: {
      adapter: PORTABLE_ADAPTER_ID,
      profile: PORTABLE_PROFILE_V1,
      house: house.name,
      ...
      apply_state: "WITHHELD — plan-only family, no writer exists",
    },
  };
  return plan;
}
