/* ════════════════════════════════════════════════════════════════════════
   ...
   ── HOW IT IS REACHED ──────────────────────────────────────────────────
   As of v8.25 applyDispatchFor("monaco-portable") returns "portable" and
   applySlice.ts calls this writer by name, handing it the exact staged
   keeper bytes and the production storage boundary
   (privateKeeperStorage.ts). Before v8.25 the family was withheld and this
   file was exercised by direct controlled tests only. The founder's Apply
   click on a contradiction-free, exact-hash plan is the act that reaches
   it in production; nothing else does.
 ...
 * contract as applyMonacoPlanSlice; applySlice.ts hosts it by name since
 * the v8.25 release.
 *
...
export async function applyPortablePlanSlice(plan, client, opts = {}) {
  ...
  /* Defence in depth beneath the route's plan-bound gate (v8.25): a plan
     whose own hashed bytes do not state Apply is enabled was generated
     while the family was plan-only, and this writer will not execute it
     no matter who calls. Strict boolean, no wording, checked before any
     read of the database. */
  if (plan.apply?.enabled !== true)
    stop("plan_apply_disabled", "this plan's own bytes do not state Apply is enabled - it was generated while the family was plan-only; generate a fresh plan under the released code");
