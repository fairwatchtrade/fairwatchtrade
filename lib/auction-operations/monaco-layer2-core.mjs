  ...
  const specs = [
    {
      ...
      price_basis_statement: quarantined
        ? "Website Result (Premium) values are source-native on a TTC premium basis and remain quarantined; no realized price is recorded from this source."
  ...
  if (sale.official_result_source) {
    specs.push({
      ...
      price_basis_statement:
        "Official auction results in EUR including buyer's premium and VAT; recorded with price_basis 'other' because the schema vocabulary has no premium+VAT value.",
...
/* THE QUARANTINE LAW LIVES HERE. A sold row earns a realized price only from
   the official result sheet. ET36 sold rows have none, so they ingest with a
   NULL price — the schema permits a sold outcome without price facts, and
   the plan summary counts every withheld row so nothing is silent. */
export function wantedLayer2Result(row, sale) {
  const priced = row.outcome === "sold" && row.official_premium_vat_eur !== null;
  return {
    sale_outcome: row.outcome,
    price_realized: priced ? row.official_premium_vat_eur : null,
    currency: priced ? sale.currency : null,
    price_basis: priced ? "other" : null,
    result_date: sale.date,
    source_key: priced ? "official_results_pdf" : "landing",
  };
}
