...
/* ── Watch Detail opening geometry: hero and right rail never share pixels ──
   ...
   right rail's painted left edge, and the photo arrows (which live inside
   the hero) must therefore stay in photo territory.
...
const STAGE_VH = num(/aspect-square max-h-\[(\d+)vh\] w-full items-center justify-center \[container-type:size\]/, gallery, "stage ceiling") / 100;
assert.match(gallery, /width: `min\(100cqw, calc\(100cqh \* \$\{heroAspect\}\)\)`/);
const ARROW = /className=\{`\$\{arrowClass\} (left|right)-3`\}/g;
assert.equal((gallery.match(ARROW) ?? []).length, 2, "both resting arrows are inset 12px inside the hero wrapper");
assert.match(gallery, /const arrowClass =\s*"absolute top-1\/2 z-10 flex h-11 w-11/);
...
function layout(vw, vh, aspect) {
  ...
  const nextRight = heroRight - ARROW_INSET;
  const prevLeft = heroLeft + ARROW_INSET;
  return { doc, cell, cellLeft, cellRight, rail, railLeft, galleryW, heroW, heroLeft, heroRight, nextRight, prevLeft, gridRight: cellRight + GAP + rail };
}
...
test("the photo arrows stay inside the photograph and out of the rail", () => {
  for (const vw of WIDTHS) for (const vh of HEIGHTS) for (const a of ASPECTS) {
    const m = layout(vw, vh, a);
    assert.ok(m.nextRight <= m.heroRight && m.nextRight - ARROW_SIZE >= m.heroLeft, `${vw}x${vh}: next arrow outside hero`);
    assert.ok(m.prevLeft >= m.heroLeft, `${vw}x${vh}: prev arrow outside hero`);
...
test("the photograph stays whole and the mechanism unchanged", () => {
  ...
  const heroBlock = gallery.slice(gallery.indexOf('data-listing-hero=""'), gallery.indexOf("{/* Previous"));
  ...
  const stage = /<div className="flex aspect-square max-h-\[\d+vh\] w-full items-center justify-center \[container-type:size\]">/.exec(gallery);
