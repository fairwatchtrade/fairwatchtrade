...
{
  const aggregateBody = integritySrc.slice(
    integritySrc.indexOf("export async function aggregateIntegrityForListing")
  );
  const regateBody = routeSrc.slice(
    routeSrc.indexOf("async function regateHeldListing"),
    routeSrc.indexOf("async function sendListingLiveEmail")
  );
  const statusDecisionBody = routeSrc.slice(
    routeSrc.indexOf("decide initial lifecycle status"),
    routeSrc.indexOf("const row: Record<string, unknown>")
  );
  ok(
    "RT25 provider constant absent from coverage, regate, and status decisions",
    !aggregateBody.includes("AUBREY_EXACT_HASH") &&
      !regateBody.includes("AUBREY_EXACT_HASH") &&
      !regateBody.includes("ensureExactHashAttempts") &&
      !statusDecisionBody.includes("AUBREY_EXACT_HASH")
  );
}
