-- ============================================================================
-- ROLLBACK for 20260730210000_v2_attestation_activation.sql
--
-- Restores public.submit_listing_for_review(uuid) to the EXACT pre-activation
-- definition, captured verbatim from production via pg_get_functiondef on
-- 2026-07-30 immediately before the activation migration was authored.
-- Function-only; the activation migration writes no data, so there is nothing
-- else to reverse. Repeatable in both directions.
--
...
comment on function public.submit_listing_for_review(uuid) is
  'Dealer submission + attestation stamp (v1 frame only — pre-activation definition, restored by rollback).';
