-- ============================================================================
-- Saved-search watcher — dialColor meaning support.
--
-- The Browse smart-search correction adds a `dialColor` meaning kind to
-- lib/search/parse.ts ("i want a blue dial" → Dial Color: Blue). The paired
-- implementation law (Search Flight 1) requires the SQL watcher to evaluate
-- the same vocabulary: without this branch, a saved Search storing a
-- dialColor meaning would pass through the watcher non-restrictively and
-- notify on every published listing regardless of dial color.
--
-- Family matching is a whole-word test against the stored dialColorType:
-- Blue matches "Blue" and "Abyss Blue", never "Champagne". Grey and Gray are
-- one family. Mirrors matchesDialColorFamily() in lib/search/parse.ts.
-- ============================================================================
