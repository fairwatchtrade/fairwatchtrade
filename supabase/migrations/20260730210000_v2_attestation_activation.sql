-- ============================================================================
-- Marketplace Money Truth — v2 ATTESTATION ACTIVATION
--
-- Replaces ONLY public.submit_listing_for_review(uuid). The fingerprint frame
-- is now selected by the row's own currency:
--
--   asking_currency NULL -> v1: the 13-field expression below, byte-identical
--                           to every fingerprint ever stamped before this
--                           migration (it IS the pre-existing expression,
--                           not a reimplementation).
--   asking_currency set  -> v2: version frame + the same 13 fields + currency
--                           as field 14, computed by the ONE canonical v2
--                           implementation (listing_attestation_fingerprint_v2,
--                           Stage A) — never by a second copy of the framing
--                           logic.
--
-- A v2 stamp with NULL currency is structurally unreachable: the only path to
-- fingerprint_v2 requires asking_currency IS NOT NULL.
--
-- Soundness: the row is locked FOR UPDATE and the stamping UPDATE runs after
-- the computation, touching only non-fingerprint fields — fingerprint_v2 reads
-- the same transaction snapshot, so it hashes exactly the truth being stamped.
--
-- CREATE OR REPLACE preserves the function's owner and grants. No tables, no
-- data, no other functions. Stage D pairing is NOT in this migration.
--
-- Rollback: scripts/v2-attestation-activation.rollback.sql (the captured
-- pre-change definition, verbatim).
--
