-- ============================================================================
-- Saved-search watcher — excludeDialColor meaning support.
--
-- The minus-operator extension teaches the parser negated dial colors
-- ("-pink" → Exclude Dial Color: Pink). The paired implementation law
-- requires the watcher to enforce the same exclusion. The companion
-- `unsupportedExclusion` kind (an exclusion the allowlist does not
-- recognize) deliberately receives NO branch here: unknown kinds fall
-- through non-restrictively, which is exactly its contract — visible in the
-- product, never filtering, mirroring matchesMeaning('unsupportedExclusion').
--
-- Family matching is the same whole-word test as dialColor: excluding Pink
-- rejects "Pink" and "Salmon Pink", never "Champagne". Grey/Gray converge.
-- ============================================================================
...
begin
  ...
  loop
    ...
    if kind = 'brand' then
      ...
      -- Whole-word family identity against dialColorType: Blue matches
      -- "Blue" and "Abyss Blue", never "Champagne". Grey/Gray converge.
      -- Mirrors matchesDialColorFamily() in lib/search/parse.ts.
      if lower(val) = 'grey' then
        if not (lower(coalesce(d->>'dialColorType', '')) ~ '\mgrey\M'
                or lower(coalesce(d->>'dialColorType', '')) ~ '\mgray\M') then
          return false;
        end if;
      else
        if not (lower(coalesce(d->>'dialColorType', '')) ~ ('\m' || lower(val) || '\M')) then
          return false;
        end if;
      end if;

    elsif kind = 'excludeDialColor' then
      -- The same family law, inverted. Mirrors
      -- matchesMeaning('excludeDialColor') in lib/search/parse.ts.
      if lower(val) = 'grey' then
        ...
        if lower(coalesce(d->>'dialColorType', '')) ~ ('\m' || lower(val) || '\M') then
          return false;
        end if;
      end if;
