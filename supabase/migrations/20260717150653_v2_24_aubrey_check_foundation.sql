-- ═══════════════════════════════════════════════════════════════════════
-- v2.24 — The Aubrey Check foundation (additive; enforcement stays OFF)
-- 1. listings.integrity_hold_reason — WHY a listing sits in pending_review.
--    Discriminates system-releasable integrity holds (results_pending /
--    provider_unavailable) from finding_review (founder-only exit) and from
--    NULL (dealer/founder queue — never system-released).
-- 2. listings.seller_clarification_note — founder-authored, ≤320 chars,
--    written ONLY on Request Clarification (listing → draft). Distinct
--    product concept from rejection_reason, which retains rejection-only
--    meaning with no exception. Seller-readable (SELECT is table-wide);
--    NOT seller-writable (v2.21 column-specific UPDATE grant excludes it).
-- 3. submit_listing_for_review() — byte-faithful to v2.21b except ONE line:
--    resubmission now also clears seller_clarification_note (D2 ruling).
-- Rollback: DROP the two columns; re-apply the function body from
-- 20260717013058_v2_21b_photo_url_normalization.sql.
-- ═══════════════════════════════════════════════════════════════════════
ALTER TABLE public.listings
  ADD COLUMN integrity_hold_reason text
    CONSTRAINT listings_integrity_hold_reason_check
    CHECK (integrity_hold_reason IN ('results_pending','provider_unavailable','finding_review')),
  ADD COLUMN seller_clarification_note text
    CONSTRAINT listings_seller_clarification_note_len
    CHECK (char_length(seller_clarification_note) <= 320);
